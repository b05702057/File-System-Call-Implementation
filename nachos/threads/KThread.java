...
public class KThread {
	 ...
	 * current thread on the ready queue, so that it will eventually be rescheduled.
	 * 
	 * <p>
	 ...
	 * This thread must be the current thread.
	 * 
	 * <p>
	...
	public void join() {
		...
		if (this.status != statusFinished) {
			Lib.assertTrue(this.parentThread == null);  // a thread can only be joined once
	...
	private static class PingTest implements Runnable {
		...
		public void run() {
			for (int i = 0; i < 5; i++) {
				...
				KThread.yield();  // the thread yield to give away the resources
			}
		}
	...
	public static void joinTest2() {
		char[] characters = new char[] {'C', 'D', 'E', 'F'}; 
		KThread child1 = new KThread (new printCharacters(characters)).setName ("forked thread1");
		System.out.print("A");
		child1.fork ();
		System.out.print("B");
		child1.join ();
		...
		System.out.println("The result should be ABCDEFG.");
	}

	private static class printCharacters implements Runnable {
		printCharacters(char[] characters) {
			this.characters = characters;
		}
		public void run () {
			KThread child2 = new KThread (new printCharacter(characters[2])).setName ("forked thread2");
	...
	private static class printCharacter implements Runnable {
	...
	public static void selfTest() {
		...
		new KThread(new PingTest(1)).setName("forked thread").fork();  // put the first PingTest in the ready queue
		new PingTest(0).run();
	 ...
	 * Used to deterministically compare threads.
	 */
	...
	// there can be only one current thread, so the functions that are related to the current thread are static
	...
	private static KThread idleThread = null;
 	...
 	private KThread parentThread = null;  // not null if the thread is joined
}
